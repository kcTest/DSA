package com.zkc.design;

/**
 * 假设你是一个专业的狗仔，参加了一个 n 人派对，其中每个人被从 0 到 n - 1 标号。在这个派对人群当中可能存在一位 “名人”。
 * 所谓 “名人” 的定义是：其他所有 n - 1 个人都认识他/她，而他/她并不认识其他任何人。
 * 现在你想要确认这个 “名人” 是谁，或者确定这里没有 “名人”。
 * 而你唯一能做的就是问诸如 “A 你好呀，请问你认不认识 B呀？” 的问题，
 * 以确定 A 是否认识 B。你需要在（渐近意义上）尽可能少的问题内来确定这位 “名人” 是谁（或者确定这里没有 “名人”）。
 * 在本题中，你可以使用辅助函数 bool knows(a, b) 获取到 A 是否认识 B。
 * 请你来实现一个函数 int findCelebrity(n)。
 * 派对最多只会有一个 “名人” 参加。若 “名人” 存在，请返回他/她的编号；若 “名人” 不存在，请返回 -1。
 * <p>
 * 示例 1:
 * 输入: graph = [
 * [1,1,0],
 * [0,1,0],
 * [1,1,1]
 * ]
 * 输出: 1
 * 解释:
 * 有编号分别为 0、1 和 2 的三个人。graph[i][j] = 1 代表编号为 i 的人认识编号为 j 的人，
 * 而 graph[i][j] = 0 则代表编号为 i 的人不认识编号为 j 的人。 “名人” 是编号 1 的人，因为 0 和 2 均认识他/她，但 1 不认识任何人。
 */
public class RelationSln {
	public static void main(String[] args) {
		
	}
	
	private static class Relation {
		/**
		 * 普通方式
		 * 0分别与0-n-1对比，1分别与1-n-1对比.....
		 * 因为从头开始  如果i认识j， i位置直接排除 范围缩小，如果i不认识j， j被排除 范围缩小。
		 * 一次遍历中每次对比都能排除一个位置。
		 * <p>
		 * 从0开始遍历
		 * 初始认为i=0为可能是明星
		 * 如果i认识i+1 说明i不是明星，此时认为i+1是明星,再看i+1位置与后面相邻位置的情况
		 * 如果i不认识i+1 说明i+1不是明星,i仍然可能是明星，再看i位置与i+1后面相邻位置的情况
		 * 每次更新可能是明星的位置
		 * 遍历完成后 最后一个保留下来的位置i不会认识后面的所有人,明星自然不会在其后面出现，所以i位置也是唯一可能的明星,。
		 * <p>
		 * <p>
		 * 然后再检查i位置是否也不认识i位置前面的人。
		 * 最后检查所有人是否都认识i位置的人。
		 */
		public int findCelebrity(int n) {
			int star = 0;
			for (int i = 0; i < n; i++) {
				if (knows(star, i)) {
					//更新
					star = i;
				}
			}
			//补充条件1  i不认识其它人
			for (int i = star - 2; i >= 0; i--) {
				if (knows(star, i)) {
					//如果认识 唯一可能是明星的位置也确认不是了
					return -1;
				}
			}
			//条件2 所有人认识i
			for (int i = 0; i < n; i++) {
				if (!knows(i, star)) {
					//有人不认识i 明星不存在
					return -1;
				}
			}
			return star;
		}
		
		/**
		 * 不需要提交 消除代码错误
		 */
		private boolean knows(int i, int star) {
			return true;
		}
		
	}
}
